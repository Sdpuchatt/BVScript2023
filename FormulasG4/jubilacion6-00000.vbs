...
SIN_DESCUENTO = LEGAJO.CONCEPTO("VLEG_SINDES") 'NO
MODALIDAD_CONTRATACION = LEGAJO.CONCEPTO("CPT_00040") '8
ACTIVIDAD_AFIP = LEGAJO.CONCEPTO("CPT_00010") '1
SITUACION_REVISTA = LEGAJO.CONCEPTO("CPT_00035") '1
GRUPO_RIESGO_COVID = LEGAJO.CONCEPTO("VLEG_GRIESG") 'N

IMPONIBLE_1_ANTERIORES = LEGAJO.CONCEPTO("CPT_50100") '0
IMPONIBLE_1 = LEGAJO.CONCEPTO("CPT_50200") '1.157.112,83
IMPONIBLE_1_SAC_ANTERIORES = LEGAJO.CONCEPTO("CPT_50105") '0
IMPONIBLE_1_SAC = LEGAJO.CONCEPTO("CPT_50400") '0

PORCENTAJE_JUBILACION = LEGAJO.CONCEPTO("VCPT_60000_CONST1")*-1 '-0.111
PISO_JUBILACION = LEGAJO.CONCEPTO("VCPT_60000_CONST2")          '35.603,99
DESCUENTOS_JUB_ANTERIORES = LEGAJO.CONCEPTO("CPT_59999")  '0
...
CPT_DIAS_TRABAJADOS = LEGAJO.CONCEPTO("CPT_00101") '30
...
ELSE
    ...
    SSQL = SSQL + " CASE WHEN SJMLGH_FCHING <= "&PRIMER_DIA_MES&" THEN "&PRIMER_DIA_MES&" ELSE SJMLGH_FCHING END," 'cuantos dias trabajo?
...
END IF
    ...
    DIAS_TRABAJADOS_SAC = LEGAJO.CONCEPTO("CPT_31000")'0  'Sólo utilizo los "Días trabajados en el semestre" para prorratear el piso de Jubilación en liquidaciones SAC/AJSAC (Según visita del 01-09-2022)
'---------------------------------------------------------------------------------------------------------------
...
IF (DIAS_TRABAJADOS <= 30 AND DIAS_TRABAJADOS <> 0) THEN
    ...
    ELSE
        PISO_JUBILACION = ((PISO_JUBILACION/30)*DIAS_TRABAJADOS) '35603.99
    END IF
END IF
...
IF (REMUNERATIVO <> 0 AND IMPOSAC = 0) THEN
    ...
    BASE_IMPONIBLE_1_COMPARACION = IMPONIBLE_1_ANTERIORES + IMPONIBLE_1  'Sueldo basico 1.100.000
